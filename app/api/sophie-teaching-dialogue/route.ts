...
/**
 * Ask Sophie (GPT-4o) to respond in the teaching dialogue
 */
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are Sophie, an AI learning assistant for conversation quality analysis.
...
async function extractRefinedLearning(
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'system',
        content: `You are extracting the final learning from a teaching dialogue between a user and Sophie (AI).
...
Extract the REFINED learning that came from this dialogue. Return JSON:
{
  "category": "price_objection|timing_objection|trust_concern|context_maintenance|message_style|followup_strategy|general_ethos|other",
